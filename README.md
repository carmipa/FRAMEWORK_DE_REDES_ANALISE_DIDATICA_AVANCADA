...
- blocos CLI Cisco por roteador (LAN, seriais, **EIGRP** com AS configurável, DHCP pool, senhas/VTY e `clock rate` típicos de laboratório Packet Tracer);
- exportação:
  - **Lab** — `.txt` consolidado só com scripts IOS para colar no Packet Tracer;
  - **Lab** — `.zip` com `GUIA_MONTAGEM_PACKET_TRACER.txt` (ler primeiro: pacote, DCE/serial, colagem no PT), consolidado, `configs_individuais/`, `LAB_TOPOLOGY.mermaid` e `README_LAB.txt`;
